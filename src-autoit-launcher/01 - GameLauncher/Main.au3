...
#AutoIt3Wrapper_Icon=..\icons\output.ico
#AutoIt3Wrapper_Outfile=..\..\..\ADVMENU\ADVMENU\GameLauncher.exe
#AutoIt3Wrapper_Res_Description=GameLauncher2019
#AutoIt3Wrapper_Res_Fileversion=5.0.0.851
...
;-----------------------------------------------------------------------------------
;Checar encipitação
If Not _CRYPT_CHECK_SAVE_KEY($O__GetSerial, $O__UserKey2, $O__ENCRYPTKEY2,$i__Obs2) Then Exit
...
Global $RETTICK_CHECK 				= _TICK_UPDATE()
...
While $LOOP
	...
	;VERIFICA A CRIPTAÇÃO A CADA 5 minutos
	If (_TICK_UPDATE() - $RETTICK_CHECK >= 300000) Then
		if (Not _CRYPT_CHECK_SAVE_KEY($O__GetSerial, $O__UserKey2, $O__ENCRYPTKEY2,$i__Obs2)) then Exit
		_TICK_RESET($RETTICK_CHECK)
		_CALL_WRITE_LOG("!Verificou criptografia!")
